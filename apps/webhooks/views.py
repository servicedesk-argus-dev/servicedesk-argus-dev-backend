...
def _default_assignment_group(organization: Organization) -> Team | None:
    team_scope = Team.objects.filter(
        Q(organization=organization) | Q(organization__isnull=True),
        is_active=True,
    )
    for team_name in ("Infra Team", "DevOps Team", "Software Team"):
        team = (
            team_scope
            .filter(name__iexact=team_name)
            .order_by("organization_id", "name")
            .first()
        )
        if team:
            return team
    return team_scope.order_by("organization_id", "name").first()
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            incident = Incident.objects.create(
                ...
                assignment_group=_default_assignment_group(organization),
